...
const RELEASES: Release[] = [
    ...
    groups: [
      {
        ...
        items: [
          ...
          { en: 'Two more things were needed, and both come from a lesson this engine already knew. A pinned example shows the missing behaviour — without it the model had only ONE way of answering without notes under its eyes, the refusal, and copied it. And a deterministic net catches what the instruction does not: on a message we ourselves classified as conversational, a refusal to inform is certainly wrong, so it never reaches the visitor. Same mechanism as the reflex re-greeting stripped in the engine: at 230M, what an instruction fails to get, a deterministic pass gets.',
            fr: 'Deux choses de plus étaient nécessaires, et les deux viennent d’une leçon que ce moteur connaissait déjà. Un exemple épinglé MONTRE le comportement qui manquait — sans lui le modèle n’avait qu’UNE façon de répondre sans fiche sous les yeux, le refus, et il la recopiait. Et un filet déterministe rattrape ce que la consigne n’obtient pas : sur un message que nous avons nous-mêmes classé conversationnel, un refus de renseigner est certainement faux, donc il n’atteint pas le visiteur. Même mécanisme que la re-salutation réflexe retirée dans le moteur : à 230 M, ce qu’une consigne n’obtient pas, un traitement déterministe l’obtient.' },
          { en: 'Measured, with a new benchmark that replays the reported transcript in order on a single conversation — the broken-record effect only shows in sequence. 3/11 before. 25/33 over three rounds with the instruction alone (the record was broken: “hello”, “are you ok?”, “ALLO?” at 3/3, and only one-word interjections and the turn right after a legitimate refusal still fell through). 33/33 with the net, of which only 4 catches: the model gets 29 of them on its own. The benchmark counts the catches separately on purpose — a harness that hides its own safety net measures nothing. The RAG benchmark stays at 12/12 in both languages: the added example degraded nothing.',
            fr: 'Mesuré, avec un banc qui rejoue la transcription signalée dans l’ordre sur une seule conversation — l’effet de disque rayé ne se voit qu’en séquence. 3/11 avant. 25/33 sur trois tours avec la seule consigne (le disque était cassé : « hello », « ça va ? », « ALLO ? » à 3/3, et seules les interjections d’un mot et le tour suivant un refus légitime passaient encore à travers). 33/33 avec le filet, dont 4 rattrapages seulement : le modèle en obtient 29 tout seul. Le banc compte les rattrapages à part volontairement — un harnais qui cache son propre filet ne mesure rien. Le banc RAG reste à 12/12 dans les deux langues : l’exemple ajouté n’a rien dégradé.' },
          { en: 'A guardrail on the build, after it bit: a plain `npm run build:sdk` silently rewrote `public/sdk-0.1.3.js`, one hour after that version was published on npm. That file must stay byte-identical to the npm package of the same number — it is what an integrator pins so their widget never changes under their feet. The build now refuses to overwrite a frozen file whose content would differ, and says to bump the version instead. It had already happened once, on 0.1.1, which had to be restored from the published tarball.',
  ...
  {
    date: { en: 'August 19, 2026', fr: '19 août 2026' },
    tagline: {
      ...
      fr: 'La génération d’image est 3,6× plus rapide et sort de bêta. Puis un audit du même jour a trouvé ce que la vitesse cachait : un défaut desktop qui pointait un modèle que ce moteur ne sait pas exécuter, huit combinaisons format×résolution qui dégradaient l’image en silence, et des réductions GPU carrément fausses sur les puces Intel et Mali. Tout est corrigé — et les modèles que nous publions sont désormais vérifiés par empreinte avant d’être chargés.',
    },
    groups: [
      {
        title: { en: 'Image generation: ×3.6, and out of beta', fr: 'Génération d’image : ×3,6, et hors bêta' },
        items: [
          { en: 'The 3×3 convolutions now compute 8 output channels per workgroup, sharing one weight load instead of re-reading it per channel: ×3.05 on a full generation (17.0 s → 5.6 s). The 1×1 convolutions got the same treatment two hours later: 4.80 s, ×3.58 against the starting point.',
