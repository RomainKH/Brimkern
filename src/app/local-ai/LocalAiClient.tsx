    ...
    knowledgeBudget: 1200
      ...
      <p style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '12px 0 0', flexWrap: 'wrap' }}>
        ...
        <span style={{ color: 'var(--text-muted)', fontSize: 12.5 }}>
          {t('Features: embeddable chat widget, headless sessions, custom system prompt, few-shot examples, local knowledge base (RAG: answers from YOUR content, selected locally, nothing sent anywhere), and local tools — arithmetic, today’s date, or your own functions, executed in your page and handed to the model as facts. embed() returns a handle — drive it, unmount it, subscribe to its events — the widget follows your page (light/dark/auto theme, corner, size, every label overridable), and every answer can name the notes it came from. Fully private and runs on the visitor’s GPU.',
            'Fonctionnalités : widget de chat intégrable, sessions headless, prompt système sur mesure, exemples few-shot, base de connaissances locale (RAG : répond sur VOTRE contenu, sélectionné en local, rien n’est envoyé nulle part), et outils locaux — l’arithmétique, la date du jour, ou vos propres fonctions, exécutées dans votre page et données au modèle comme des faits. embed() rend une poignée — pilotez-le, démontez-le, abonnez-vous à ses événements — le widget suit votre page (thème clair/sombre/auto, coin, taille, chaque libellé surchargeable), et chaque réponse peut nommer les fiches dont elle vient. 100 % privé sur le GPU du visiteur.')}
        </span>
      </p>
