...
async function loadMotionModules(engine: WebGpuEngine, url: string, onProgress?: (s: string) => void): Promise<Map<string, MotionModule>> {
  onProgress?.('Téléchargement du module motion…');
  ...
  onProgress?.(`${out.size} modules motion résidents.`);
  return out;
}
...
export async function loadVideoGenerator(
  ...
  const { map: unetST, unetCfg: brikCfg } = await brikImageToMap(engine, urls.unet, onProgress, 'UNet');
  const unetCfg = { ...SD15_UNET, ...(brikCfg || {}) } as UnetCfg;
  const unetW = loadUnetWeights(unetST, unetCfg);
  const { map: clipST } = await brikImageToMap(engine, urls.clip, onProgress, 'CLIP');
  const clip = new ClipTextEncoder(engine, loadClipWeights(clipST, CLIP_L), CLIP_L);
  const motions = await loadMotionModules(engine, urls.motion, onProgress);
  onProgress?.('VAE (TAESD)…');
  ...
  const generate = async (prompt: string, opts: { seed?: number; frames?: number; size?: number; onProgress?: (s: string) => void } = {}): Promise<VideoFrames> => {
    ...
    const t0 = performance.now();
    prog?.('Encodage du prompt (CLIP-L)…');
    ...
    for (let i = 0; i < sched.timesteps.length; i++) {
      prog?.(`Débruitage ${i + 1}/${sched.timesteps.length} (${F} frames)…`);
      const scaled = latents.map((l) => sched.scaleModelInput(l, i));
      const stepPace: UnetPace = { ...pace, onBlock: (b) => prog?.(`Débruitage ${i + 1}/${sched.timesteps.length} (${F} frames) — bloc ${b}/${blocksTotal}…`) };
  ...
  const enrich = async (prompt: string, prog?: (s: string) => void): Promise<string> => {
    ...
    if (!enricher) {
      ...
      enricher = await loadPromptEnricher(engine, urls.lfm, prog ?? onProgress);
    }
