// Node test for the BWP codec (no GPU needed): run with
//   node --experimental-strip-types src/lib/bwp/codec.test.ts
...
import { BWP_ALIGN, alignUp } from './format';
...
// 4. Shard packing: alignment + multi-tensor decode.
{
	...
	check('shard length is 16-aligned', buffer.length % BWP_ALIGN === 0, `len=${buffer.length}`);
	...
	for (const t of tensors) {
		...
		check(`tensor '${t.name}' shard=${e.shard} offset=${e.offset} aligned`, e.offset % BWP_ALIGN === 0);
	}
