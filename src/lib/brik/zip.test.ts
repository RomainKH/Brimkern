// Node test for the store-only ZIP writer/reader. Run via `npm run test:brik`.
...
const manifestJson = JSON.stringify({ format: 'brik', version: 1, hello: 'wörld €' });
...
const entries = [
	...
	{ name: 'shard-0000.brik', data: shard0 },
	{ name: 'shard-0001.brik', data: shard1 },
];
...
check('binary shard matches byte-for-byte', eq(byName.get('shard-0000.brik')!, shard0));
check('empty shard round-trips', eq(byName.get('shard-0001.brik')!, shard1));
