...
`}return e}var wt="https://esm.sh/@huggingface/transformers@4.2.0",et={"lfm2.5-230m":"https://huggingface.co/romainkh14/LFM2.5-230M_BRIK/resolve/main/lfm25-230m-q4.brik"},At={F16:"f16",F32:"f32",Q4W:"q4",Q8W:"q8",Q3W:"q3"};async function Ut(g,t){let r=new Fe;if(!await r.init())throw new Error("WebGPU indisponible sur ce navigateur.");await r.selfValidate(),t("t\xE9l\xE9chargement du mod\xE8le\u2026");let e=await Je(g),n=e.manifest,a=n.tensors["token_embd.weight"],s={arch:{...n.config,arch:"lfm2",vocab:a?a.nElems/n.config.d:0},tensors:Object.fromEntries(Object.entries(n.tensors).map(([h,d])=>[h,{dtype:At[d.type]??d.type,shape:d.shape,nElems:d.nElems,shard:0,offset:d.offset,byteLength:d.bytes}])),shards:[{id:0,file:"",byteLength:0}],chat:{template:"chatml",stopTokenIds:[7]}},i=async h=>{let d=n.tensors[h];if(!d)throw new Error(`tenseur absent : ${h}`);return e.source.bytes(d.offset,d.bytes)};t("tokenizer\u2026");let u=await import(wt),c=new u.PreTrainedTokenizer(JSON.parse(e.tokenizer.json),JSON.parse(e.tokenizer.config)),f={encode:h=>Array.from(c(h).input_ids.data,d=>Number(d)),decode:h=>c.decode(h,{skip_special_tokens:!0})},l=new Se(r,s,i);return t("poids sur le GPU\u2026"),await l.load(f),l}function kt(g){if(document.getElementById("bk-style"))return;let t=document.createElement("style");t.id="bk-style",t.textContent=`
  ...
  `,document.head.appendChild(t)}function Pt(g){if(!g)return"#c72c1e";if(/^#[0-9a-fA-F]{3,8}$/.test(g))return g;try{if(typeof CSS<"u"&&CSS.supports("color",g)&&!/[{};()]/.test(g))return g}catch{}return"#c72c1e"}function tt(g){let t=Pt(g.accent),r=g.title||"Assistant",e=g.maxTokens||220;kt(t);let n=document.createElement("button");n.className="bk-fab",n.setAttribute("aria-label","Ouvrir le chat"),n.textContent="\u{1F4AC}";let a=document.createElement("div");a.className="bk-panel",a.innerHTML=`
    <div class="bk-hd"><span class="bk-dot"></span><span>${Gt(r)}</span><button class="bk-x" aria-label="Fermer">\xD7</button></div>
    ...
    <div class="bk-note">IA locale \u2014 tourne sur votre GPU, aucune donn\xE9e envoy\xE9e.</div>`,document.body.appendChild(n),document.body.appendChild(a);let s=a.querySelector(".bk-msgs"),i=a.querySelector(".bk-in"),o=a.querySelector(".bk-send"),u=[],c=!1,f=null,l=(m,v)=>{let k=document.createElement("div");return k.className=`bk-m ${m==="user"?"bk-u":"bk-a"}`,k.textContent=v,s.appendChild(k),s.scrollTop=s.scrollHeight,k},h=m=>{let v=s.querySelector(".bk-status")||l("assistant",m);v.classList.add("bk-status"),v.textContent=m};g.greeting&&(u.push({role:"assistant",content:g.greeting}),l("assistant",g.greeting));let d=()=>{if(!f){let v=g.model&&(g.model.startsWith("https://")||/^http:\/\/(localhost|127\.0\.0\.1)[:/]/.test(g.model))?g.model:et[g.model||"lfm2.5-230m"]||et["lfm2.5-230m"],k=l("assistant","Initialisation\u2026");k.classList.add("bk-status"),f=Ut(v,O=>{k.textContent=O}).then(O=>(k.remove(),O)).catch(O=>{throw k.textContent="Erreur : "+(O?.message||O),f=null,O})}return f},p=async()=>{let m=i.value.trim();if(!m||c)return;c=!0,o.disabled=!0,i.value="",u.push({role:"user",content:m}),l("user",m);let v=l("assistant","\u2026");try{let k=await d(),O=Ze(u,"lfm2",g.system||""),q="";await(k.residentAvailable?.()?k.generateResident.bind(k):k.generate.bind(k))(O,e,M=>{q=M,v.textContent=M||"\u2026",s.scrollTop=s.scrollHeight},void 0,{sample:!0,temperature:.7,topK:40,repeatPenalty:1.3}),v.textContent=q||"(vide)",u.push({role:"assistant",content:q})}catch(k){v.textContent="Erreur : "+(k?.message||String(k))}finally{c=!1,o.disabled=!1,i.focus()}};n.onclick=()=>{a.classList.toggle("bk-open")&&(i.focus(),d())},a.querySelector(".bk-x").onclick=()=>a.classList.remove("bk-open"),o.onclick=()=>{p()},i.onkeydown=m=>{m.key==="Enter"&&!m.shiftKey&&(m.preventDefault(),p())}}function Gt(g){return g.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}window.Brimkern={embed:(g={})=>{document.body?tt(g):window.addEventListener("DOMContentLoaded",()=>tt(g))}};})();
