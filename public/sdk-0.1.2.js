...
--- NOTES ---
${t}
--- END OF NOTES ---`}function ir(l){let e=Array.isArray(l)?l:[l],r=[];for(let t of e)typeof t=="string"&&t.trim()?r.push({text:t}):t&&typeof t=="object"&&typeof t.text=="string"&&t.text.trim()&&r.push({title:t.title,text:t.text});return r}At();async function Jr(l){let{LocalBackend:e}=await Promise.resolve().then(()=>(Pt(),Qr));if(l!==!0)return new e;try{let{WorkerBackend:r}=await Promise.resolve().then(()=>(Xr(),Yr)),t=new r;return await t.ready(),t}catch(r){return console.warn("[brimkern] Web Worker indisponible : inf\xE9rence sur le thread principal",r),new e}}Ut();var Zn=typeof self<"u"&&typeof self.importScripts=="function"&&typeof document>"u";Zn&&Promise.resolve().then(()=>(Zr(),Jn));var nt=null,Bt=null,Gt;function Ne(){return nt||(nt=Jr(Gt).then(l=>(Bt=l,l))),nt}var ei=()=>Bt?.kind??"pending";function qt(l){if(l.workerUrl&&Ir(l.workerUrl),l.worker!==void 0){if(nt&&Gt!==l.worker){console.warn("[brimkern] option `worker` ignor\xE9e : le backend est d\xE9j\xE0 d\xE9marr\xE9 et partag\xE9 par la page.");return}Gt=l.worker}}var ti=`
Answer briefly and honestly. If you do not know something, say so: never invent facts or details.
You have no tools and no internet access: never emit tool calls, reply in plain text only.`;function tn(l){let e=(l.system||"You are a helpful assistant.")+ti,r=s=>s.flatMap(o=>[{role:"user",content:o.user},{role:"assistant",content:o.assistant}]);if(!l.knowledge)return{system:()=>e,userTurn:s=>s,pinned:r(l.examples||[])};let t=rr(ir(l.knowledge)),n=l.knowledgeBudget??1200,i=l.lang?l.lang==="fr":l.system?/[àâäéèêëîïôöùûüç]|\b(?:bonjour|salut|vous|tu|réponds|conseiller|boutique|aide|aidez|client|magasin)\b/i.test(l.system):!1,a=i?e+`
...
The user message may include reference notes between --- markers. When it does, answer from those notes and quote their figures exactly. When it says no note matches, say you do not have that information.`;return{system:()=>a,userTurn:s=>{let o=dt(nr(s,t,n),s,i).trim();return o?`${o}

Question: ${s}`:s},pinned:r([...ri(i),...l.examples||[]])}}function ri(l=!1){let e=(t,n)=>({title:t,text:n,doc:0}),r=(t,n)=>`${dt(t,void 0,l).trim()}
